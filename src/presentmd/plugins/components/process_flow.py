...
class ProcessFlowComponent:
    @property
    def name(self) -> str: return "process-flow"
    def parse_metadata(self, content: str, attrs: dict) -> dict:
        return {
            "attrs": attrs,
            "items": _parse_process_flow_items(content)
        }
    def render_html(self, content: str, metadata: dict, render_inline: callable) -> str:
        ...
        for idx, item in enumerate(items):
            ...
            icon_html = f'<div class="pmd-process-flow-icon">{render_inline(icon)}</div>' if icon else ""
            text_html = f'<div class="pmd-process-flow-text">{render_inline(step_text)}</div>' if step_text else f'<div class="pmd-process-flow-text">{idx + 1}</div>'
            desc_html = f'<div class="pmd-process-flow-desc">{render_inline(item["desc"])}</div>' if item.get("desc") else ""
            label_html = f'<div class="pmd-process-flow-label">{render_inline(item["label"])}</div>' if item.get("label") else ""
            
            items_html.append(
                f'<div class="pmd-process-flow-item{steps_class}"{steps_attr} data-color="{escape(color)}">'
                f'  {icon_html}'
                f'  {text_html}'
                f'  {label_html}'
                f'  {desc_html}'
        ...
        return (
            f'<pmd-process-flow{steps_container_attr}>\n'
            f'  {"".join(items_html)}\n'
            f'</pmd-process-flow>'
        )
